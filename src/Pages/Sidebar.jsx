...
const Sidebar = ({ children, onLogout }) => {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="border-t mt-auto p-2 bg-white shrink-0 flex flex-col gap-1">
        {/* Logout Button */}
        <Button
          variant="ghost"
          className={`w-full justify-start ${isSidebarExpanded ? 'px-2' : 'px-1'} h-10 text-red-600 hover:text-red-700 hover:bg-red-50`}
          onClick={onLogout}
        >
          <LogOut size={18} />
          {isSidebarExpanded && <span className="ml-2 text-sm">Logout</span>}
        </Button>
